...
ufhe_priv_keyset ufhe_new_priv_keyset(ufhe_params params){
  ...
  res->tlwe = tlwe_new_key(prms.n, prms.lwe_std_dev);
  res->trlwe = trlwe_new_key(prms.N, prms.k, prms.rlwe_std_dev);
  res->extracted_trlwe = tlwe_new_key(prms.k*prms.N, prms.lwe_std_dev);
